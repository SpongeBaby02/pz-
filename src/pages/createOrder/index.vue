...
const showTimeConfirm = (item) =>{
    const dateStr = item.selectedValues.join('-')//以对象的形式转为字符串并且-连接
...
const componionConfirms = computed(()=>{
...
const submit = async() =>{
    ...
    for(const i of params){//遍历数组
      if(!form[i]){//如果form中少了一项
        showNotify({ message: '请完整填写每一项' });
        return
      }

    }
    const{data: orderRes } = await proxy.$api.createOrder(form) //用async和await去调用api接口
